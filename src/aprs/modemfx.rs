...
impl ModemFx {
    ...
    pub fn new(filename:&str)-> ModemFx {
	...
	fn write_freq(&mut self , freq:f32, duration:f32){
        let amplitude = 1.3;
		let cycles_per_bit = ((SAMPLE_RATE as f32)*duration) as i32;
        let delta : usize= (512 * (freq as usize) << 7) / ((cycles_per_bit * BAUD) as usize);
		for _t in (0 .. cycles_per_bit).map(|x| x as f32 ) {
			let tpl = self.phase.overflowing_add(delta);
			self.phase = tpl.0;
			let sample:i32 = _sine_table[(self.phase >> 7) & 0x1FF] as i32 * 0x80 - 0x4000;
			self.writer.write_sample((sample as f32 * amplitude) as i16).unwrap();
		}
	}
